...
<!-- <img src="attachments/SSL/CPC_equation.png" alt="CPC equation" width="700" class="center"/>
 -->
By jointly optimizing the representations and their predictions, CPC learns latent representations **z**[*t*] and context representations **c**[*t*] that encode different aspects of the input speech, such as phonemic units and speaker identities, a in well-separable manner {cite}`oord2018representation`. 

Note that the standard CPC uses waveforms instead of spectral features as the input, and therefore a CNN encoder is applied to map the speech into the latent space **z**[*t*] (see Fig. 2). However, the core CPC learning mechanism can also be appied to spectral feature inputs, in which case MLP encoder would be typically instead of a CNN (e.g., {cite}`chung2019unsupervised`; see also *"Choosing between waveform and acoustic feature inputs"* section below). 
...
Given the fast pace of the development of SSL methods, it may be difficult to identify the most suitable method for a particular use case. This is where standardized benchmarks for SSL performance can be of use. One such benchmark is the Speech processing Universal PERformance Benchmark (SUPERB) benchmark {cite}`yang2021superb` and its multilingual extension ML-SUPERB {cite}`shi2024mlsuperb`. Such benchmarks usually consist of several downstream tasks in which representations from a pre-trained SSL encoder are tested as input features. For example, the tasks of the original SUPERB include automatic speech recognition, phoneme recognition, speaker identification, speaker verification, speaker diarization, speech emotion recognition, and many more. Performance of different SSL methods in these tasks are listed on a leaderboard [https://superbbenchmark.github.io/#/leaderboard](https://superbbenchmark.github.io/#/leaderboard), enabling straightforward comparison of alternative methods. 
